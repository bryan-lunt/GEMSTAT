...
ExprPar ParFactory::create_expr_par(const vector<double>& pars, const ThermodynamicParameterSpace in_space) const
{
      ...
      if ( expr_model.modelOption == CHRMOD_UNLIMITED || expr_model.modelOption == CHRMOD_LIMITED || expr_model.modelOption == DIRECT || expr_model.modelOption == MARKOV || expr_model.modelOption == RATES )
      {
          for ( int i = 0; i < _nFactors; i++ )
          {
...
ExprPar ParFactory::load_old(istream& fin){
  ...
  for ( int i = 0; i < expr_model.getNFactors(); i++ )
  {
      ...
      if ( expr_model.modelOption == CHRMOD_UNLIMITED || expr_model.modelOption == CHRMOD_LIMITED || expr_model.modelOption == DIRECT || expr_model.modelOption == MARKOV || expr_model.modelOption == RATES ) fin >> tmp_par.repEffects[i];
  }
...
ExprPar::ExprPar( const vector< double >& pars, const IntMatrix& coopMat, const vector< bool >& actIndicators, const vector< bool >& repIndicators, int _nSeqs ) : factorIntMat()
{
    ...
    // set the repression effects
    if ( modelOption == CHRMOD_UNLIMITED || modelOption == CHRMOD_LIMITED || modelOption == DIRECT || modelOption == MARKOV || modelOption == RATES)
    {
        for ( int i = 0; i < _nFactors; i++ )
        {
            if ( repIndicators[i] )
...
//    os.precision( 50 );
    ...
    for ( int i = 0; i < nFactors(); i++ )
    {
        ...
        if ( modelOption == CHRMOD_UNLIMITED || modelOption == CHRMOD_LIMITED || modelOption == DIRECT || modelOption == MARKOV || modelOption == RATES) os << "\t" << repEffects[i];
        os << endl;
    }
